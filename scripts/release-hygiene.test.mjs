...
test('distributed ignore policy protects fresh community checkouts', t => {
 ...
 for(const file of ['.env','.env.staging','.omx/state.json','.codebase-memory/graph.db','.agents/skills/x/SKILL.md','.ai/skills/x/SKILL.md','.codex/state.json','node_modules/x','dist/x','.next/x','__pycache__/x.pyc','.claude/settings.local.json','test-results/x','secrets.pem']) {
  assert.equal(spawnSync('git',['check-ignore','-q','--',file],{cwd:dir,env}).status,0,file);
 }
 for(const file of ['.env.example','src/App.tsx','AGENTS.md','pnpm-lock.yaml']) {
